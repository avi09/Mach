...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
        y=Xi.iloc[i]
        ff=np.exp(wt*similarity(x,(y.iloc[0:(len(y)-1)])))
        n1=n1+y[-1]*ff
        n2=n2+ff
...
def similarity(a,b):
    ...
    y2=np.mean(b)
    for i in range(x):
        an=an+((a.iloc[i]-y1)*(b.iloc[i]-y2))
    an=an/(x*np.std(a)*np.std(b))
    return an
...
for i in range(len(train1_90.index)):
    testDeltaP90=np.append(testDeltaP90,computeDelta(w,test_90.iloc[i],train1_90))
for i in range(len(train1_180.index)):
    testDeltaP180=np.append(testDeltaP180, computeDelta(w,test_180.iloc[i],train1_180))
for i in range(len(train1_360.index)):
    testDeltaP360=np.append(testDeltaP360,computeDelta(w,test_360.iloc[i],train1_360))
